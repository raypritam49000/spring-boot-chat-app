...
public class TokenProvider {
	...
	public String generateToken(Authentication authentication) {
		...
		String jwt = Jwts.builder().setIssuer("the_codebreaker").setIssuedAt(new Date())
				.setExpiration(new Date(new Date().getTime() + 8640000)) // expire after 24hr
				.claim("email", authentication.getName()).signWith(key, SignatureAlgorithm.HS256) // Sign with the key
																									// and algorithm
				.compact();
		return jwt;
	}
