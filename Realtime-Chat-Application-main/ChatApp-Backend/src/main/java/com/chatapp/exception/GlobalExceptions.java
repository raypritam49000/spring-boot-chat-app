...
public class GlobalExceptions {
	...
	public ResponseEntity<ErrorDetails> MethodArgumentsNotValidExceptionHandler(MethodArgumentNotValidException e3,
			WebRequest req) {
	...
	public ResponseEntity<ErrorDetails> NoHandlerFoundExceptionHandler(NoHandlerFoundException e4, WebRequest req) {
		ErrorDetails error = new ErrorDetails("No handler avalble for this endpoint", e4.getMessage(),
				LocalDateTime.now());
		return new ResponseEntity<ErrorDetails>(error, HttpStatus.BAD_REQUEST);
	}
